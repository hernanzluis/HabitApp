...
| [workflow.md](workflow.md) | Proceso de trabajo entre Luis, Claude y Code: quién hace qué, cuándo interviene Luis manualmente en Supabase, formato de los prompts, y la regla de evaluar cobertura de `tests/` en cada cambio (qué lo dispara, cuándo aplica, cuándo anotar un hueco en `tests/README.md`) | Al planificar cómo se va a ejecutar una tarea, si hay dudas sobre quién debe dar cada paso, o antes de dar por cerrado cualquier cambio que toque RLS/RPC/tablas/reglas de negocio |
