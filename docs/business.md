...
- **Stripe solo en web:** cuando se active la monetización, el checkout de Stripe estará únicamente en habitteam.app (panel web). La app móvil no tendrá compras in-app (IAP) para evitar la comisión del 15-30% de Apple/Google y la complejidad de dos sistemas de facturación en paralelo. La app mostrará el plan activo y un CTA que redirige a la web para gestionar la suscripción.
  ...
  > ⚠️ **Contradicción sin resolver (detectada en auditoría, no corregida automáticamente):** el texto legal público en `habitteam-web/src/pages/Terms.jsx` (secciones "Planes y precios" y "Cancelación", ES y EN) dice justo lo contrario: que los cobros se hacen "exclusivamente a través de la App Store (Apple) o Google Play (Google)" y que la cancelación se gestiona desde ahí. Esto es una decisión de negocio/legal, no algo que se pueda arreglar solo con código — hay que decidir cuál de los dos modelos es el vigente y corregir el documento o el texto legal (el que esté desfasado) en consecuencia.
