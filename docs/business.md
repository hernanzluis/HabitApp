...
- **Stripe solo en web:** cuando se active la monetización, el checkout de Stripe estará únicamente en habitteam.app (panel web). La app móvil no tendrá compras in-app (IAP) para evitar la comisión del 15-30% de Apple/Google y la complejidad de dos sistemas de facturación en paralelo. La app mostrará el plan activo y un CTA que redirige a la web para gestionar la suscripción.
  ...
  > ✅ **Contradicción resuelta (2026-09-16):** `habitteam-web/src/pages/Terms.jsx` (secciones "Planes y precios" y "Cancelación", ES y EN) afirmaba que los cobros se hacían "exclusivamente a través de la App Store (Apple) o Google Play (Google)". Se corrigió para reflejar la realidad: plan Familiar gratuito con límites de uso, plan Plus de pago "próximamente" (método de cobro y condiciones se comunicarán cuando esté disponible), sin ninguna afirmación sobre App Store/Google Play. La landing (`Home.jsx`) también se actualizó: la tarjeta del plan Plus muestra un badge "Próximamente" en vez de precio, con el CTA deshabilitado.
